...
// Helper para pegar instancia do supabase
...
function initChecklist(osId, veiculoId, clienteId) {
    ...
    if (osId) {
        ...
        if (existe) {
            ChecklistState.checklistAtual = existe;
...
function preencherFormularioChecklist() {
    ...
    s('hodometro',   c.hodometro);
    s('observacoes', c.observacoes);
...
function criarNovoChecklist(osId, veiculoId, clienteId) {
    ChecklistState.checklistAtual = {
        id: null, // sera preenchido pelo Supabase
        os_id: osId || null,
        ...
        luzes_avarias: [],
        inspecao_visual: { lataria:'', pneus:'', vidros:'', interior:'' },
        servicos: [],
        pecas: [],
        observacoes: '',
        fotos: [],
...
async function salvarChecklist() {
    ...
    const payload = {
        ...
        hodometro:          gv('hodometro'),
        nivel_combustivel:  nc ? parseInt(nc.value) : 4,
        tipo_combustivel:   combustTipos,
        itens:              coletarItensChecklist(),
        inspecao_visual: {
            lataria:  gv('inspecaoLataria'),
            pneus:    gv('inspecaoPneus'),
            vidros:   gv('inspecaoVidros'),
            interior: gv('inspecaoInterior')
        },
        servicos:           coletarServicos(),
        pecas:              coletarPecas(),
        status_regulacao:   gv('statusRegulacao'),
        seguradora:         gv('seguradora'),
        regulador:          gv('regulador'),
        data_regulacao:     gv('dataRegulacao') || null,
        observacoes:        gv('observacoes'),
        fotos:              ChecklistState.checklistAtual.fotos || [],
    ...
    if (idx >= 0) AppState.data.checklists[idx] = resultado;
    else AppState.data.checklists.push(resultado);
...
// Carregar checklists do Supabase (chamado pelo app.js se necessario)
...
function setupNavigacaoTeclado() {
    document.addEventListener('keydown', function(e) {
        ...
        if (t.classList.contains('input-peca-valor'))    { e.preventDefault(); adicionarLinhaPeca(); }
            ...
            e.preventDefault();
            var next = t.parentElement.parentElement.querySelector(
...
function atualizarResumoFinanceiro() {
    ...
    var tr2 = sv.filter(x=>x.regulado).reduce((s,x) => s+(parseFloat(x.valor)||0), 0)
             + pc.filter(x=>x.regulado).reduce((s,x) => s+(parseFloat(x.valor)||0), 0);
...
function coletarServicos() {
    return Array.from(document.querySelectorAll('#tabelaServicos tr'))
        .map(tr => ({
            descricao: tr.querySelector('.input-servico-desc') ? tr.querySelector('.input-servico-desc').value : '',
            valor:     parseFloat(tr.querySelector('.input-servico-valor') ? tr.querySelector('.input-servico-valor').value : 0) || 0,
            regulado:  !!(tr.querySelector('input[type="checkbox"]') && tr.querySelector('input[type="checkbox"]').checked)
...
function coletarPecas() {
    return Array.from(document.querySelectorAll('#tabelaPecas tr'))
        .map(tr => ({
            descricao: tr.querySelector('.input-peca-desc') ? tr.querySelector('.input-peca-desc').value : '',
            valor:     parseFloat(tr.querySelector('.input-peca-valor') ? tr.querySelector('.input-peca-valor').value : 0) || 0,
            regulado:  !!(tr.querySelector('input[type="checkbox"]') && tr.querySelector('input[type="checkbox"]').checked)
...
async function preencherChecklistDemoCompleto(gerarPdfAoFinal) {
    ...
    [
        {descricao:'Troca de oleo e filtros',valor:'80.00'},
        {descricao:'Revisao de freios completa',valor:'120.00'},
        {descricao:'Substituicao correia dentada',valor:'250.00'}
    ].forEach((s,i) => tServ && tServ.appendChild(criarLinhaServico({id:Date.now()+i,...s,regulado:i%2===0})));
    [
        {descricao:'Filtro de oleo (Qtd: 1)',valor:'45.00'},
        {descricao:'Filtro de ar (Qtd: 1)',valor:'38.00'},
        {descricao:'Filtro de combustivel (Qtd: 1)',valor:'52.00'},
        {descricao:'Filtro de cabine (Qtd: 1)',valor:'42.00'},
        {descricao:'Oleo motor 5W30 (1L) (Qtd: 4)',valor:'35.00'},
        {descricao:'Vela de ignicao (Qtd: 4)',valor:'28.00'},
        ...
        {descricao:'Bomba dagua (Qtd: 1)',valor:'210.00'},
...
async function gerarPDF() {
    ...
    async function getLogoBase64() {
        const logoSrc = (!AppState.oficina.logo || AppState.oficina.logo.indexOf('via.placeholder.com')!==-1) ? 'logo-default.png' : AppState.oficina.logo;
        if (logoSrc.startsWith('data:')) return logoSrc;
        try {
            const blob = await (await fetch(logoSrc)).blob();
            return await new Promise(r => { var fr=new FileReader(); fr.onload=()=>r(fr.result); fr.readAsDataURL(blob); });
        } catch(e) {
            try { const blob = await (await fetch('logo-default.png')).blob(); return await new Promise(r => { var fr=new FileReader(); fr.onload=()=>r(fr.result); fr.readAsDataURL(blob); }); } catch(e2) { return ''; }
        }
    }
    ...
    function drawHeader(){
        ...
        if(logoBase64&&/^data:image\//.test(logoBase64)){var lf=logoBase64.indexOf('image/jpeg')!==-1||logoBase64.indexOf('image/jpg')!==-1?'JPEG':'PNG';doc.addImage(logoBase64,lf,22.8,22.8,18.4,12.4,undefined,'FAST');}
        else{doc.setFont('helvetica','bold');doc.setTextColor(205,25,25);doc.setFontSize(8);doc.text('LOGO',32,30,{align:'center'});}
        doc.setFontSize(10.5);doc.setTextColor(0,0,0);doc.text(oficina.nome,45,25);
    ...
    function drawBadges(x,y,w,h,items){
        ...
        items.forEach(item=>{
            ...
            if(item.marcado){doc.setFillColor(corPrimariaRgb[0],corPrimariaRgb[1],corPrimariaRgb[2]);doc.setTextColor(255,255,255);}
    ...
    if(fotosComp.length>0){
        fotosComp.slice(0,3).forEach((f,i)=>{var fx=24+i*53,fy=143;doc.setDrawColor(200,40,40);doc.roundedRect(fx,fy,51,36,1,1);doc.addImage(f.url,'JPEG',fx+0.5,fy+0.5,50,35,undefined,'FAST');});
        fotosComp.slice(3,9).forEach((f,i)=>{var fx=24+i*27.2,fy=182;doc.setDrawColor(200,40,40);doc.roundedRect(fx,fy,25,18,1,1);doc.addImage(f.url,'JPEG',fx+0.5,fy+0.5,24,17,undefined,'FAST');});
    }
    drawFooter(true);

    // PAG 2 — PECAS + SERVICOS
    ...
    var headSty={fillColor:corPrimariaRgb,textColor:255,fontStyle:'bold',fontSize:7};
    var footSty={fillColor:corPrimariaRgb,textColor:255,fontStyle:'bold',fontSize:8};
