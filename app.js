...
function renderTrialCountdownBanner() {
    ...
    banner.innerHTML = `<span>${texto}</span><button onclick="window._abrirUpgradeDoCountdown()" style="background:rgba(255,255,255,.25);color:#fff;border:1px solid rgba(255,255,255,.5);border-radius:20px;padding:3px 10px;font-size:12px;font-weight:700;cursor:pointer;white-space:nowrap;">Assinar</button><button onclick="document.getElementById('trialCountdownBanner').style.display='none'" style="background:none;border:none;color:rgba(255,255,255,.7);cursor:pointer;font-size:16px;line-height:1;padding:0 2px;">&times;</button>`;
    document.body.appendChild(banner);
}
window._abrirUpgradeDoCountdown = function() { renderTrialPopup(AppState.oficina); };
...
window._logoutBloqueio = async function() {
    ...
    localStorage.removeItem('checkauto_user'); sessionStorage.removeItem('checkauto_user');
...
async function checkAuth() {
    try {
        ...
        let usuario = null; let ultimoErro = null;
        const retryDelayMs = [0, 250, 600, 1200];
        for (const delayMs of retryDelayMs) {
            if (delayMs > 0) await sleep(delayMs);
            const { data, error } = await supabase.from('usuarios').select('id, nome, email, role, oficina_id, usuario_login, ativo').eq('id', session.user.id).maybeSingle();
            if (!error && data) { usuario = data; break; }
            ultimoErro = error || null;
        }
        if (!usuario) {
            const cachedUser = getStoredUser();
            if (cachedUser) {
                AppState.user = { id: cachedUser.id, email: cachedUser.email || session.user.email, nome: cachedUser.nome || session.user.email.split('@')[0], role: cachedUser.role || 'user', oficina_id: cachedUser.oficina_id || null, loginTime: new Date().toISOString() };
        ...
        AppState.user = {
            ...
            usuario_login: usuario.usuario_login || null,
...
async function loadFromSupabase() {
    try {
        ...
        AppState.data.ordensServico = (ordensServico || []).map(os => ({ ...os, clienteId: os.cliente_id, veiculoId: os.veiculo_id, valorTotal: os.valor_total, dataConclusao: os.data_conclusao, servicos: (os.os_servicos || []).map(s => ({ id: s.id, descricao: s.descricao, valor: s.valor })) }));
        AppState.data.agendamentos = (agendamentos || []).map(a => ({ ...a, clienteId: a.cliente_id, veiculoId: a.veiculo_id, tipoServico: a.tipo_servico }));
        AppState.data.contasPagar   = contasPagar || [];
        AppState.data.contasReceber = (contasReceber || []).map(c => ({ ...c, osId: c.os_id, osNumero: c.os_numero, pagadorTipo: c.pagador_tipo, pagadorNome: c.pagador_nome, formaPagamento: c.forma_pagamento, parcelasTotal: c.parcelas_total, parcelasRecebidas: c.parcelas_recebidas, valorRecebido: c.valor_recebido }));
        AppState.data.contasFixas = (contasFixas || []).map(c => ({ ...c, valorMensal: c.valor_mensal, diaVencimento: c.dia_vencimento, pagoEsteMes: c.pago_este_mes }));
    ...
    } catch (e) { console.error('Erro ao carregar dados do Supabase:', e); showToast('Erro ao carregar dados! Verifique a conexao.', 'error'); }
}
...
function shouldShowTrialPopupToday(oficinaId) {
    ...
    localStorage.setItem(key, today); return true;
}
...
async function enforceTrialAndPopup() {
    ...
    if (trialVencido) {
        ...
        renderTrialBloqueado(AppState.oficina); return;
    }
    if (plano === 'TRIAL' && shouldShowTrialPopupToday(oficinaId)) renderTrialPopup(AppState.oficina);
    ...
    if (typeof carregarOficinaDoDB === 'function') {
        try {
            ...
            if (oficina && typeof aplicarWhiteLabel === 'function') {
                ...
                AppState.oficina = Object.assign({}, AppState.oficina, { id: oficina.id, status: oficina.status, plano: oficina.plano || 'TRIAL', trial_ate: oficina.trial_ate || null, created_at: oficina.created_at || null });
            }
...
function navigateTo(page) {
    ...
    if (pageElement) {
        ...
        else if (page === 'funcionarios' && typeof renderFuncionarios === 'function') renderFuncionarios();
...
function renderRecentOS() {
    ...
    tbody.innerHTML = list.map(os => `<tr><td><strong>${esc(os.numero)}</strong></td><td>${esc(os.cliente)}</td><td>${esc(os.veiculo)}</td><td>${getStatusBadge(os.status)}</td><td>${formatDate(os.data)}</td><td><button class="btn-icon" onclick="viewOS('${os.id}')" title="Ver"><i class="fas fa-eye"></i></button></td></tr>`).join('');
}
